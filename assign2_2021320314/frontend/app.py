...
def index():
    ...
    """
    res = requests.get(f"{BACKEND_URL}/api/message")
    data = res.json()
    curr_msg = data.get("message", "")

    message = curr_msg
    timestamp = ""

    if "(updated at" in curr_msg:
        parts = curr_msg.rsplit("(updated at", 1)
...
def update():
    ...
    requests.post(f"{BACKEND_URL}/api/message", json={"message": new_msg})
    return redirect("/")
